...
1. **音效类型**：游戏有6种核心音效
   ...
   - **过关音效** (`win`)：关卡通过时播放
   - **失败音效** (`lose`)：球出界时播放
   - **放置线段音效** (`line_place`)：线段确认时播放
   - **撤销线段音效** (`line_undo`)：撤销线段时播放
...
|--------|-----------|---------------------|--------|
| **画线反弹系统** | 输入 ← | 触发反弹音效 | Approved |
| **光点收集系统** | 输入 ← | 触发收集音效 | Approved |
| **出界检测系统** | 输入 ← | 触发失败音效 | Approved |
| **星级评价系统** | 输入 ← | 触发过关音效 | Approved |
| **场景管理** | 输入 ← | 切换背景音乐 | Approved |
| **UI系统** | 双向 | 音量控制接口 | Approved |
